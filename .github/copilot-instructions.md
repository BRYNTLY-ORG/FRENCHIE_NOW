...
- Runner label matrix: ordinary macOS/ARM64 jobs use `[self-hosted, macOS, ARM64, num5]` or repo-approved `num5-macos`; Docker-backed Linux ARM64 jobs use `[self-hosted, Linux, ARM64, num5, num5-linux]`; CodeQL jobs use `[self-hosted, macOS, ARM64, num5, num5-codeql]`. Do not introduce `ubuntu-latest`, `macos-latest`, or other GitHub-hosted labels unless the workflow documents an explicit approved exception.
...
- Treat `gk ai`, `docker ai`, Copilot, LM Studio, and other model outputs as advisory until verified by code, diffs, logs, tests, or read-back commands.
